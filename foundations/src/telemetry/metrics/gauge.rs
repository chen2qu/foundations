...
impl RangeGauge {
    ...
    fn get_values(&self) -> (u64, u64, u64) {
        ...
        // SAFETY: It is possible that current == min, and another thread decremented current before we read its value,
        // but has not yet decremented min. So, enforce the invariant that min <= current.
        let min = std::cmp::min(current, self.min.swap(current, Ordering::AcqRel));
        // SAFETY: Same caveat as above applies to max.
        let max = std::cmp::max(current, self.max.swap(current, Ordering::AcqRel));
        // It is possible that the current value was incremented or decremented between us getting the
        // value in step 1 and setting the min/max values in step 2.
